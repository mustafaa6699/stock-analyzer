          ...
          <div ref={resultsRef}>
            ...
            <Card mb={12} style={{padding:"24px 26px",background:"linear-gradient(135deg,rgba(16,185,129,0.06),rgba(59,130,246,0.06))",border:"1px solid rgba(16,185,129,0.2)"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:14,marginBottom:18}}>
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                    <div style={{fontSize:30,fontWeight:900}}>{mkt.symbol}</div>
                    <div style={{padding:"3px 10px",borderRadius:50,fontSize:11,fontWeight:700,background:"rgba(16,185,129,0.15)",color:C.green2,border:"1px solid rgba(16,185,129,0.3)"}}>🔴 LIVE</div>
                  </div>
                  <div style={{fontSize:13,color:C.text2,marginTop:4}}>{mkt.companyName}</div>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginBottom:16}}>
                {[
                  ...
                  {l:"أعلى اليوم",v:"$"+fmt(mkt.dayHigh)},
                  {l:"أدنى اليوم",v:"$"+fmt(mkt.dayLow)},
                  {l:"أعلى 52 أسبوع",v:"$"+fmt(mkt.weekHigh52)},
                  {l:"حجم التداول",v:fmt(mkt.volumeM,1)+"M"},
                ].map((s,i)=>(
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {[
                  ...
                  {label:"💹 Yahoo Finance",url:`https://finance.yahoo.com/quote/${mkt.symbol}`},
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:12}}>
              {[
                ...
                {label:"حجم التداول",val:data.volumeStatus,sub:fmt(mkt.volumeM,1)+"M سهم",color:C.text},
              ].map((m,i)=>(
                <Card key={i}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
                ...
                <div style={{display:"flex",gap:6}}>
                  {[["price","السعر"],["rsi","RSI"],["vol","الحجم"]].map(([m,l])=>(
                    <button key={m} onClick={()=>setChartMode(m)} style={{padding:"5px 13px",borderRadius:8,fontSize:12,fontWeight:600,border:`1px solid ${chartMode===m?C.blue:"rgba(99,179,237,0.12)"}`,background:chartMode===m?"rgba(59,130,246,0.1)":"transparent",color:chartMode===m?C.blue2:C.text2,cursor:"pointer",fontFamily:"inherit"}}>{l}</button>
                  ))}
                </div>
              </div>
            ...
            <Card mb={12}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:8}}>
                {(data.indicators||[]).map((ind,i)=>{
                  ...
                  return <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 13px",borderRadius:10,background:C.bg3,border:"1px solid rgba(99,179,237,0.12)"}}><span style={{fontSize:12,color:C.text2}}>{ind.name}</span><span style={{fontSize:13,fontWeight:700,color:c}}>{a} {ind.value}</span></div>;
                })}
              </div>
            </Card>
            ...
            {/* ANALYSIS */}
            <Card mb={12}>
              <div style={{fontSize:15,fontWeight:700,marginBottom:12}}>📋 توصية الخبير — مبنية على بيانات حقيقية</div>
